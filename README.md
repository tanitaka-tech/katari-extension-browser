...
ツールバーの「★」で開閉するポップアップ（ページを押し出さない）。現在のページが
どちらかに登録済みなら「★」、未登録なら「☆」になる。開いている間だけネイティブ webview が
自動で隠れる（webview は常に最前面に描かれるため）。閉じれば元に戻る。
...
URL をその場で編集**する。`×` で削除、ドラッグで並べ替え。

## 履歴メニュー（🕘）

ツールバーの「🕘」で開閉するポップアップ。**ブラウザタブごと**の訪問履歴（新しい順、最大 20 件）を出し、
